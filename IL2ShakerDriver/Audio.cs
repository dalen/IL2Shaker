...
internal class Audio : ISampleProvider
{
    ...
    public void Start(string? deviceName, int latency, CancellationToken token)
    {
        ...
        Logging
            .At(this)
            .Information(
                "Outputting to '{Output}' with {Latency}ms latency",
                device.FriendlyName,
                latency
            );
        Latency = latency;
    ...
    {
        return new MMDeviceEnumerator().EnumerateAudioEndPoints(
            DataFlow.Render,
            DeviceState.Active
        );
    }
    ...
    public void UpdateSettings(ISettings settings)
    {
        ...
        if (switchedState || previousdB != MasterVolume.dB)
            Logging
                .At(this)
                .Verbose("{State}: {Rel}dB", Enabled ? "Enabled" : "Disabled", MasterVolume.dB);
    ...
    private void SetSimSpeed(SimSpeed simSpeed)
    {
        if (SimSpeed != simSpeed)
        {
            ...
            string state =
                pausing ? " - Pausing shaker output"
                : resuming ? " - Resuming shaker output"
                : "";
