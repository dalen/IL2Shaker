...
internal class HitsReceived : Effect
{
    ...
    public HitsReceived(ISampleProvider source, Audio audio)
        : base(source, audio) { }
    ...
    protected override void OnEventDataReceived(Event eventData)
    {
        if (eventData is HitEvent hitEvent)
        {
            Logging
                .At(this)
                .Debug(
                    "Tick = {tick}, Offset = {offset}, Energy = {energy}",
                    hitEvent.Tick,
                    hitEvent.Offset,
                    hitEvent.Force.Length()
                );

            float keRatio =
                (MathF.Sqrt(hitEvent.Force.Length()) - MinSqrKineticEnergy)
